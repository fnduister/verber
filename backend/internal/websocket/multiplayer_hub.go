...
type MultiplayerHub struct {
	...
	// Callback when user goes fully offline (no active connections remain)
...
func (h *MultiplayerHub) Run() {
	for {
		...
		case client := <-h.unregister:
			...
			h.mu.Unlock()
			// After releasing the lock, broadcast offline status and invoke callback
			if offlineUserID > 0 {
				if h.OnUserOffline != nil {
					h.OnUserOffline(offlineUserID)
				} else {
					h.BroadcastPresenceUpdate(offlineUserID, false)
				}
			}
